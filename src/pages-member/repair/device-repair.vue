...
<template>
  <div>
    <app-view :hasFooter="true">
      <router-link to="/repair/choose">
        <div class="padding-m bg-white flex" style="align-items: flex-start">
          <div class="product-img-box border-radius border">
            <div class="product-img">
              <img class="img-thumbnail" v-lazy="$filters.img((userDevice && userDevice.productImage ? userDevice.productImage : ''), '!wh100') "/>
            </div>
          </div>
          <div v-if="formData.deviceId" class="flex-item padding-left-m">
            <div class="font-bold font-title text-wrapper"> {{ (userDevice && userDevice.deviceName) || '无设备名称' }} </div>
            <div class="font-s font-light margin-top-s">
              <div class="margin-bottom-s">产品货号：{{ userDevice && userDevice.itemCode ? userDevice.itemCode : '无' }}</div>
              <div v-if="userDevice && userDevice.deviceType !== 'L'">购买日期：{{ (userDevice && userDevice.purchasedDate && $filters.formatDate(userDevice.purchasedDate)) || '无' }}</div>
            </div>
          </div>

            <div v-else-if="!formData.deviceId" class="flex-item padding-left-m font-m font-bold-500 valign-center ripple" style="height: 60px;">
              点击选择要维修的设备 <i class="iconfont icon-iot-youjiantou1"></i>
            </div>
        </div>
      </router-link>
      ...
      <div class="footer flex" slot="footer">
        ...
        <div class="flex-item submit">
          <button type="button" class="btn btn-block"
                  :class="{ 'disabled-btn': !formData.deviceId, 'btn-primary': formData.deviceId }"
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        formData: {
          // 设备id
          deviceId: (this.$route.query && this.$route.query.deviceId) || null,
          // 店铺服务id
          shopServiceId: "",
          // 服务日期
...
//          newAddress: ''
    ...
    created () {
      ...
      // 获取用户设备信息
      let _deviceId = that.$route.query && that.$route.query.deviceId;
      if(_deviceId) {
        that.$http.get(`${that.$apihost}/devices/${_deviceId}`)
          .then((data) => {
            if(data) {
              that.userDevice = data;
            }
          })
      }
            ...
            if(result) {
              ...
              sysApi.validateSmsCode(that.addressData.contactMobile,that.smsCode)
                .then(() => {
                  ...
                  this.$dlg.confirm("确定提交？", function () {
