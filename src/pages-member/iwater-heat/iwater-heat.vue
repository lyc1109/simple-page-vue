  ...
  <app-view>
    <div class="iWater-main">
      ...
      <div class="purest-box" v-show="iWaterNav === 'purest'">
        <div class="purest padding-top-l">
          <!--续费按钮-->
          <div>
            <div class="pay-btn-box" v-if="iWater.deviceType == 'L'">
              <div class="pay-box flex" @click.stop="recharge()">
                <div class="flex-item">
                  <div class="font-m color-warning font-bold-700">续费 <i class="iconfont icon-iot-youjiantou1 font-m font-bold-300"></i></div>
          ...
          <div class="purestNum padding-left-l padding-right-l margin-top-l">
            <div class="beforePurest valign-center">
              <div style="width: 100%;">
                <span class="font-m font-bold-500">{{ iWater.iotDevice && iWater.iotDevice.inWaterTds ? iWater.iotDevice.inWaterTds : 0 }}</span>
                <!--<i class="font-s font-opacity">mg/L</i>-->
                <div class="font-s font-opacity">净化前TDS</div>
              </div>
            </div>
            ...
            <div class="afterPurest valign-center">
              <div style="width: 100%;">
                <span class="font-m font-bold-500">{{ iWater.iotDevice && iWater.iotDevice.outWaterTds ? iWater.iotDevice.outWaterTds : 0 }}</span>
                <!--<i class="font-s font-opacity">mg/L</i>-->
                <div class="font-s font-opacity">净化后TDS</div>
              </div>
            </div>
          </div>
          <!--其余-->
        ...
        <div class="operate-box padding-top padding-bottom bg-white">
          <ul class="flex align-center btn-gruop-box">
            ...
            <li @click.stop="switchTag('account')" class="flex-item btn-box ripple">
              ...
              <span class="font-default">净水统计</span>
            </li>
        ...
        <div class="margin-top bg-white adjust-box">
          <div class="flex padding-m border-bottom" @click.stop="openHeat('outflow')">
            <div class="flex-item">
              <i class="iconfont icon-iot-cs font-bold-500"></i>
              <span class="font-bold-500">出水量</span>
            </div>
            <div class="adjust-num font-bold-500 color-primary">{{ heatData && heatData.outflow ? heatData.outflow : 0 }}  ml</div>
          ...
          <div class="flex padding-m border-bottom" @click.stop="openHeat('tea')">
            <div class="flex-item">
              ...
              <span class="font-bold-500">泡茶</span>
            </div>
            <div class="adjust-num font-bold-500 color-danger-light">{{ heatData && heatData.tea ? heatData.tea : 0 }}  ℃</div>
            <i class="margin-left-s iconfont icon-iot-youjiantou1 font-lighter font-s"></i>
          </div>
    ...
    methods: {
      ...
      fetchData() {
        ...
        this.$http.get(`${this.$apihost}/userdevices/${this.usrDeviceId}`)
          .then((data) => {
            this.entity = data;
            this.iWater = data.device;
            this.deviceId = this.iWater.id;
            ...
            this.bookQuery = {
              ...
              address: encodeURIComponent(JSON.stringify(res.address))
            };
      ...
      switchTag(type) {
        ...
        if(_type && that.deviceId) {
          that.$router.push({ path: `/devices/${that.deviceId}/i${_type}`, query: { userDeviceEntityId: that.usrDeviceId }});
        }
      },
      /**
       * 调整面板值
