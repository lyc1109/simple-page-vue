...
<template>
  <div class="align-center">
    <div class="flex padding">
      ...
      <div class="flex-item">
        <div class="iconbox iconbox-blue" @click="updateTiming" :class="{'disabled': !isOnline}"><i class="iconfont icon-iot-ds"></i></div>
        <div class="font-s margin-top-s font-lighter">预约定时</div>
      </div>
...
<script>
  ...
  export default {
    name: "v-iwater-actions-cold",
    desc: "普通智能加热制冷水机操作小组件",
    components: {VSettingTemperature},
    props: {
      device: {
        type: Object,
        default: {
    ...
    methods: {
      ...
      // 重新配网
      restoreWiFi: function () {
        if(!this.isOnline) {
          return;
        }
        IotIwater.restoreWiFi(this.device.id);
      },
      // 更新出水温度
